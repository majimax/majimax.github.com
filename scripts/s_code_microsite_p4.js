...
var s_account="sssamsung4cn,sssamsung4mstglobal"
var s_lochref=location.href
if(s_lochref.indexOf("local.opentide.com.cn")>-1)
{
	 s_account = "sssamsungnexttest,sssamsung4mstglobaldev"; 
}
...
s.linkInternalFilters="javascript:,samsung.com,local.opentide.com.cn"
...
/************* 以下内容是从之前s_code_microsite_p3.js文件复制过来的。是蒋老师为了简便点击代码的添加而定义的语句 （2014.8.1日由jay更新）**************/
	...
	function s_control_click(vLinkTrackVars, vLinkTrackEvents, vLinkTrackValues, vType, vTypeName) {
		...
		var tempProducts = "";
		var supportPageCheck = "";
		var supportProductSeparator = "";
		...
		// Check pageTrack
		supportPageCheck = document.getElementById( "pageTrack" );	
			
		if( supportPageCheck == undefined || supportPageCheck == null || supportPageCheck == ""){
			supportPageCheck = window.parent.document.getElementById( "pageTrack" );
			if ( supportPageCheck != undefined && supportPageCheck != null && supportPageCheck != "" ){
				supportProductSeparator = window.parent.document.getElementById( "pageTrack" ).value.toLowerCase();
			}
		}else {
			supportProductSeparator = document.getElementById( "pageTrack" ).value.toLowerCase();
		}
		// End of Checking pageTrack
		...
		for(var xFI = 0; xFI < vSplitLinkTrackVars.length; xFI++) {
			if(vSplitLinkTrackVars[xFI] == "products") { 
				...
				if(tempProducts.indexOf(":") != -1){
					tempProducts = tempProducts.split(":")[1];
				}
				
				if( supportProductSeparator.indexOf("support") != -1 || SplitedURL[ 2 ].toLowerCase() == "support" ){
					s[vSplitLinkTrackVars[xFI]]= ";support:" + tempProducts;
				}else {
...
//Microsite Page Link or button Click.
function LinkClick(vName,vType)
{
	...
	s.tl(this,vType,vName);
}

function ShowPV(vName)
{
	var s2 = s_gi(s_account);

	if(vName.length==0) return;
	var PVArray=vName.split(":");
	var hier1="";
	...
	s2.pageName=vName;
	s2.hier1=hier1;
	s2.channel=PVArray[0]+':'+PVArray[1];
	s2.prop1=PVArray[0];
	s2.prop2=PVArray[0]+':'+PVArray[1];
	if(PVArray[2]) s2.prop3=PVArray[0]+':'+PVArray[1]+':'+PVArray[2];
	if(PVArray[3]) s2.prop4=PVArray[0]+':'+PVArray[1]+':'+PVArray[2]+':'+PVArray[3];
	if(PVArray[4]) s2.prop5=PVArray[0]+':'+PVArray[1]+':'+PVArray[2]+':'+PVArray[3]+':'+PVArray[4];
	...
	s2.referrer=" ";
	var s_code2=s2.t();
	if(s_code2)document.write(s_code2); 
}